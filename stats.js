(function () {
  ...
  const localSprite = id => id === 10301 ? "assets/megas/mega-zygarde.png" : sprite(id);
  let runs = [];
  let scope = "personal";
  ...
  function monImg(mon) {
    return `<img src="${localSprite(mon.id)}" alt="${mon.name}" loading="lazy"
      onerror="this.onerror=null;this.src='${sprite(mon.base_id || mon.id)}'">`;
  }
  ...
  function drawCommunityBest(run) {
    if (!run) {
      $("communityBest").innerHTML =
        '<p class="empty">The first newly recorded team will claim this spot.</p>';
      return;
    }
  ...
  async function init() {
    ["usageRegion","historyRegion","bestRegion"].forEach(id => $(id).innerHTML = regionOptions("All regions"));
    ...
    if (!DexleStats.configured) {
      ...
      drawHistory();
      drawBests();
      drawCommunityBest(null);
      return;
    }
    try {
      const [mine, community, communityBest] = await Promise.all([
        DexleStats.personalRuns(),
        DexleStats.communitySummary(),
        DexleStats.communityBestTeam().catch(() => null),
      ]);
      runs = mine;
      drawSummary(community);
      drawCommunityBest(communityBest);
