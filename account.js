(function(){
  ...
  function renderAvatarChoice(id,a){const el=$(id);if(!el)return;el.innerHTML=a?`<img class="profile-avatar" src="${avatarSprite(a)}" onerror="this.onerror=null;this.src='${sprite(a.base_id||a.id,a.shiny)}'"><span><b>${a.name}</b>${a.shiny?" · Shiny":""}</span><button type="button" class="avatar-remove" aria-label="Remove selected Pokémon">&times;</button>`:'<span class="avatar-placeholder"></span><span>No sprite selected</span>';}
  function wireAvatarSearch(inputId,shinyId,resultsId,choiceId,persist){
    const input=$(inputId),toggle=$(shinyId),results=$(resultsId),choiceEl=$(choiceId);if(!input)return;choiceEl.onclick=async e=>{if(!e.target.closest(".avatar-remove"))return;selectedAvatar=null;renderAvatarChoice(choiceId,null);if(persist){try{currentAccount.profile=await DexleStats.updateAvatar(null);message("Profile sprite removed.");}catch(err){message(err.message,true);}}};
    const draw=()=>{const q=String(input.value||"").trim().toLowerCase();if(q.length<2){results.innerHTML="";return;}const shiny=toggle.checked;const hits=avatarCatalog().filter(p=>p.name.toLowerCase().includes(q)).slice(0,30);results.innerHTML=hits.map((p,i)=>`<button type="button" class="avatar-result" data-avatar="${i}"><img src="${avatarSprite({...p,shiny})}" onerror="this.onerror=null;this.src='${sprite(p.base_id||p.id,shiny)}'"><small>${p.name}</small></button>`).join("");results.onclick=async e=>{const b=e.target.closest("[data-avatar]");if(!b)return;selectedAvatar={...hits[+b.dataset.avatar],shiny};renderAvatarChoice(choiceId,selectedAvatar);results.innerHTML="";input.value="";if(persist){try{currentAccount.profile=await DexleStats.updateAvatar(selectedAvatar);message("Profile sprite updated.");}catch(err){message(err.message,true);}}};};input.oninput=draw;toggle.onchange=async()=>{if(selectedAvatar){selectedAvatar={...selectedAvatar,shiny:toggle.checked};renderAvatarChoice(choiceId,selectedAvatar);if(persist){try{currentAccount.profile=await DexleStats.updateAvatar(selectedAvatar);message("Profile sprite updated.");}catch(err){message(err.message,true);}}}draw();};
  }
  ...
  function drawDex(){
    ...
    const forms=dex.filter(x=>!String(x.form_key).startsWith("base:")).length;
