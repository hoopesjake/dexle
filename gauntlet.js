...
function drawChallenge() {
  ...
  $("chgrid").innerHTML=`<article class="chcard daily-champion-card ${done?"complete":""}"><small>Once per day</small><b>Daily Champion Battle</b><span>One rotating Champion · 6 vs 6</span><strong class="daily-champion-reset">${done?"Completed · ":""}resets in ${reset}</strong><button class="daily-champion-action" data-daily="1">${done?"Share Your Results":"Play Today's Battle"}</button></article><button class="chcard regular-champion-card" data-regular="1"><small>Unlimited play</small><b>Region Champion</b><span>Choose one of nine full region circuits. Beat a region and earn a badge!</span><strong>Choose a Region</strong></button>`;
...
function runDailyChampion(){
  ...
  const finalTeamStats=team.reduce((total,m)=>total+statsFor(m).reduce((sum,v)=>sum+v,0),0);
